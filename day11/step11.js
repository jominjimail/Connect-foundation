...
function executeEventQueue(callstack, eventQueue) {
    if(!stack_isEmpty(callstack)){
        setTimeout(() => executeEventQueue(callstack, eventQueue), 0);
    }else if(!stack_isEmpty(eventQueue)){
...
function executeCallStack(callstack, eventQueue) {
    ...
    setTimeout(() => { executeCallStack(callstack, eventQueue)}, 0);
}
